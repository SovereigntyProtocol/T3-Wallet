...
import { convertToValidatorAddress, cosmosURL, getPublicKeyType } from '~/Popup/utils/cosmos';
...
export default function NativeChainCard({ chain, isCustom = false }: NativeChainCardProps) {
  ...
  // this useEffect is used for fetch the DID of an add
  // eslint-disable-next-line @typescript-eslint/no-empty-function

  useEffect(() => {
    // eslint-disable-next-line @typescript-eslint/require-await
    const fetchUserDid = async () => {
      if (chain?.chainId === 'sov-5') {
        const { getDid } = cosmosURL(chain);
        const fetchurl = getDid(currentAddress);
  ...
  return (
    <Container data-is-no-expanded={chain.custom === 'no-stake'}>
      ...
      {userDID ? (
        <DidContainer>
          <FirstLineLeftContainer>
            <AddressButton onClick={handleOnClickdidCopy}>{userDID}</AddressButton>
          </FirstLineLeftContainer>
