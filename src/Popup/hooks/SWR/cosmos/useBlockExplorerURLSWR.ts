...
export function useBlockExplorerURLSWR(chain: CosmosChain, config?: SWRConfiguration) {
  ...
  const getExplorerAccountURL = useCallback(
    (address?: string) => {
      ...
      return explorerBaseURL ? `${explorerBaseURL}/${chain?.chainId === 'sov-5' ? 'account' : 'address'}/${address}` : '';
    },
    [chainParams.data?.params?.chainlist_params?.explorer?.account, chain?.chainId, getExplorerURL],
  );
